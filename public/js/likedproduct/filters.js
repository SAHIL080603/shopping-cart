...
let userid2=pagination.getAttribute('userid');
let username2=pagination.getAttribute('username');
// console.log(filterform);
async function filterproducts(search,rating,min,max,sort,page){
    // console.log('inside getproducts');
    const res=await fetch(`https://shopping-cart-tbh7.onrender.com/profile/${userid2}/${username2}/likedproducts/filters?q=${search}&ratingfilter=${rating}&minpricefilter=${min}&maxpricefilter=${max}&sortfilter=${sort}`);
    ...
    for(let product of letproducts){
        ...
        if(currentUser && currentUser.likedProducts && currentUser.likedProducts.includes(product._id)){
            ...
            temp= `<a href="/products/${ product._id}/like?bool=1" class="btn like-button bg-light rounded-circle" style="color: grey;" id="like-button">
                <i  product-id="${product._i}" class="far fa-heart"></i>
            </a>`
        }
